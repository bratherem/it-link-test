...
Миграции (включая предустановленные данные — 15 объявлений) применяются автоматически при старте контейнера `app`.
...
Примените миграции (создание таблиц и наполнение 15 тестовыми объявлениями):
...
Миграция `m260626_000002_seed_car_data` добавляет 15 объявлений автомобилей (12 с техническими характеристиками, 3 без). Данные загружаются автоматически вместе с остальными миграциями.

Для сброса и повторного наполнения:

```bash
php yii migrate/down 1   # откатить сид
php yii migrate/up       # применить снова
```
